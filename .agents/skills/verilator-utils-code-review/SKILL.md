...
- In module interface files (`src/*.cppm`), `using namespace` directives must stay at block (local) scope; namespace-scope `using namespace` is reserved for internal files such as tests where it may simplify code. The only namespace-scope exceptions in interface files are the deliberate `export using namespace ::verilator_utils::data_format::interface;` re-export of the module's own public inline namespace, and the TU-scope `namespace { using namespace ::std::string_view_literals; }` that enables `"..."sv` literals file-wide (see String Formatting below).
- Prefer `::std::string_view` literals (`"..."sv`) over C-style string literals for `::std::format`/`::std::format_to` format strings, `VU_CHECK` messages, and trace file names, enabled TU-wide by the file's `namespace { using namespace ::std::string_view_literals; }`.
...
- Enable the literal operators once per module interface file with a TU-scope `namespace { using namespace ::std::string_view_literals; }` placed after the module declaration and imports. Because a using-directive makes the nominated names appear as if declared in the nearest enclosing namespace containing both the directive and `::std::string_view_literals` (the module's global scope), `"..."sv` then resolves everywhere in the TU — including class-scope member initializers (e.g. `eval_scheduler::unit_table`) and specializations in nested namespaces such as `namespace std` formatters. The directive sits inside an unnamed namespace and using-directives never cross module imports, so it stays TU-local and is never exported to importers. Do not add per-function `using namespace ::std::string_view_literals;` — it is redundant.
...
- Format strings and assertion messages use `"..."sv` string_view literals, enabled TU-wide by the single `namespace { using namespace ::std::string_view_literals; }` at the top of each interface file.
- `using namespace` at namespace scope appears only in internal files (tests) or as the two sanctioned interface-file patterns: the `export using namespace` re-export and the TU-scope `namespace { using namespace ::std::string_view_literals; }`.
